<template>
  <div class="flex flex-col gap-4 pb-10">
    ...
    <PDeferredContent
      v-for="i in restPostsTotalPages"
      :key="i"
      @load="next"
      class="flex flex-col gap-4 max-w-full"
    >
      <PostCard
        v-for="post in getPostsSlice(i)"
        :key="post.id"
        v-bind="post"
        :flat-name="`${post.flatName}`"
      />
      <template v-if="isMobile">
        ...
        <SidebarFilters v-else="isMobile" />
      </template>
    </PDeferredContent>
  </div>
</template>
...
const restPostsTotalPages = computed(
  () => restPosts.value.length / pageSize.value,
);
